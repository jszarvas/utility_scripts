...
parser.add_argument("-t", "--tree", dest="mat", help="Input phylip matrix, no distance computation")
...
if args.seq_file is None:
    if args.mat is not None:
        wdir = os.path.dirname(args.mat)
        prefix = os.path.realpath(args.mat)
    else:
...
if args.mat is None:
    ...
    outputmat = "{}.encoded.mat".format(prefix)
    try:
        f = open(args.mat, "r")
    except IOError as e:
    ...
    with open(outputmat, "w") as matfile:
        ...
        for l in f:
            if l and not l.isspace():
                ...
                seq_id = "I{:07}".format(taxaindex)
                seqid2name[seq_id] = taxaname
                matfile.write("{:<10}".format(seq_id))
                matfile.write(distances)
    f.close()
